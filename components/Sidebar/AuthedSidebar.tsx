...
function setItems() {
  ...
  const Items: Array<LinkItemProps> = [
      { name: 'Home', icon: FiHome, pagelink: '/' },
      { name: 'Games', icon: GiMeeple, pagelink: '#' },
      { name: 'Settings', icon: FiSettings, pagelink: '#' },
    ];
  return Items
}
...
export default function AuthedSidebar({ children }: { children: ReactNode }) {
...
const LogOut = () => {
  ...
  return (
    <Link onClick={logoutfunc} href={'#'} style={{ textDecoration: 'none' }} _focus={{ boxShadow: 'none' }}>
      <Flex
        align="center"
        p="4"
        mx="4"
